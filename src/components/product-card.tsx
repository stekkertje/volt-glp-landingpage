    ...
    <article className="surface-card flex flex-col overflow-hidden rounded-xl">
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <p className="text-xs text-dim">
          {product.unit} · ±{product.weeksAtStart} weken
        </p>
        <p className="text-xs tabular-nums text-dim">
          {formatEuro(pricePerWeekCents(product))}/week bij startdosis
        </p>
        {product.form === "vial" && (
          <p className="mt-1 text-[11px] text-dim">
            Bac water inbegrepen · {product.syringeCount ?? 10} spuiten optioneel
          </p>
        )}
